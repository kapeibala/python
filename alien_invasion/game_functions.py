...
def create_stars(ai_settings, screen, stars):
    ...
    for _ in range(ai_settings.num_stars):  # num_stars is a new attribute in Settings
        star = Star(ai_settings, screen)
        stars.add(star)


def update_stars(ai_settings, screen, stars):
    """Update the positions of multiple stars"""
    stars.update()

    # Remove stars that have reached the bottom
    for star in stars.copy():
        if star.rect.y >= ai_settings.screen_height:
            stars.remove(star)

    # Add new stars to maintain a constant number of stars
    while len(stars) < ai_settings.num_stars:
        create_stars(ai_settings, screen, stars)


def check_stars_bottom(ai_settings, screen, stats, sb, ship, stars):
    """检查是否有星星到达了屏幕底端"""
    screen_rect = screen.get_rect()
    for star in stars.sprites():
...
def check_bullet_star_collisions(ai_settings, screen, stats, sb, bullets, stars):
    """检查子弹和星星的碰撞"""
    collisions = pygame.sprite.groupcollide(bullets, stars, True, False)
...
def check_bullet_star_collisions(ai_settings, screen, stats, sb, bullets, stars):
    """检查子弹和星星的碰撞"""
    collisions = pygame.sprite.spritecollideany(stars, bullets)

    if collisions:
        # Handle the collision
        stats.score += ai_settings.star_points
        sb.prep_score()
        stars.sprites()[0].reset_star()
